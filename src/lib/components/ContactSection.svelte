<script lang="ts">
  import { Mail, MessageCircle, Send, Github, Linkedin, MapPin, Calendar, Clock, CheckCircle, AlertCircle, ExternalLink, Download } from 'lucide-svelte';
...
<section id="contact" class="relative min-h-screen bg-black p-4 sm:p-6 lg:p-8">
  ...
  <div class="relative z-10 max-w-7xl mx-auto">
    <div class="text-center mb-16">
      ...
      <div class="inline-flex items-center px-4 py-2 rounded-full border border-accent-main/20 bg-accent-main/5 backdrop-blur-[40px]">
        <span class="status-dot relative inline-block animate-[pulse_2.5s_ease-in-out_infinite] rounded-full bg-linear-to-br/oklch from-accent-main to-accent-mid size-2 mr-3 shadow-[0_0_12px_rgba(255,90,90,0.8)]"></span>
        <span class="text-sm font-semibold text-accent-main">Available for immediate opportunities</span>
      </div>
    </div>
      ...
      <div class="lg:col-span-2">
        <div class="card-container relative perspective-distant">
          <div class="card card-glass ease-fluid relative transform-3d overflow-hidden rounded-3xl border border-white/15 p-8 lg:p-12 backdrop-blur-[80px] backdrop-brightness-110 backdrop-saturate-200 transition-all duration-800 hover:border-white/22">
            <div class="floating-accent absolute top-6 right-6 z-10 size-1.5 animate-[float-accent_5s_ease-in-out_infinite] rounded-full bg-linear-to-br/oklch from-accent-main to-accent-mid shadow-[0_0_16px_rgba(255,90,90,0.9)]"></div>

            <div class="flex items-center mb-8">
              <div class="p-3 rounded-xl bg-white/10 backdrop-blur-[20px] mr-4">
                <MessageCircle size={24} class="text-accent-main" />
              </div>
              <div>
            ...
            <form onsubmit={handleSubmit} class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" id="company" bind:value={formData.company} class="w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white placeholder-white/50 backdrop-blur-[20px] transition-all duration-300 focus:border-accent-main/50 focus:bg-white/8 focus:outline-none focus:ring-2 focus:ring-accent-main/20" placeholder="Your company (optional)" />
                </div>
                <div>
                  <label for="subject" class="block text-sm font-semibold text-white mb-2">Subject *</label>
                  <select id="subject" bind:value={formData.subject} required class="w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white backdrop-blur-[20px] transition-all duration-300 focus:border-accent-main/50 focus:bg-white/8 focus:outline-none focus:ring-2 focus:ring-accent-main/20">
              ...
              <div>
                ...
                <textarea id="message" bind:value={formData.message} required rows={6} class="w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white placeholder-white/50 backdrop-blur-[20px] transition-all duration-300 focus:border-accent-main/50 focus:bg-white/8 focus:outline-none focus:ring-2 focus:ring-accent-main/20 resize-none" placeholder="Tell me about the opportunity, project, or how I can help..."></textarea>
              </div>

              {#if status.type !== 'idle'}
                <div class="flex items-center p-4 rounded-xl border {status.type === 'success' ? 'border-accent-main/20 bg-accent-main/5 text-accent-main' : status.type === 'error' ? 'border-red-500/20 bg-red-500/5 text-red-400' : 'border-blue-500/20 bg-blue-500/5 text-blue-400'}">
                  {#if status.type === 'success'}
                    <CheckCircle size={20} class="mr-3 flex-shrink-0" />
                  {:else if status.type === 'error'}
                    <AlertCircle size={20} class="mr-3 flex-shrink-0" />
                  {:else}
              ...
              <button type="submit" disabled={status.type === 'loading'} class="ease-snappy w-full flex items-center justify-center px-8 py-4 rounded-xl border border-accent-main/20 bg-linear-to-br/oklch from-accent-main/15 via-accent-light/10 to-accent-mid/15 text-white font-semibold transition-all duration-400 hover:border-accent-main/30 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0">
      ...
      <div class="space-y-6">
        <div class="bento-item ease-snappy relative z-2 border border-white/8 bg-linear-to-br/oklch from-white/4 via-white/1 to-white/3 rounded-2xl lg:rounded-3xl p-6 backdrop-blur-[40px] backdrop-saturate-150">
          <h3 class="text-lg font-bold text-white mb-6 flex items-center">
            <Mail size={20} class="mr-3 text-accent-main" />Contact Information
          </h3>
          <div class="space-y-4">
            {#each contactInfo as info (info.label)}
              <div class="flex items-start">
                <div class="p-2 rounded-lg bg-white/10 backdrop-blur-[20px] mr-3 mt-0.5">
                  <info.icon size={16} class="text-accent-main" />
                </div>
                <div class="flex-1">
                  <div class="text-sm font-semibold text-white mb-1">{info.label}</div>
                  {#if info.href}
                    <a href={info.href} class="text-sm text-accent-main hover:text-white transition-colors">{info.value}</a>
                  {:else}
            ...
            {#each socialLinks as social (social.label)}
              <a href={social.href} target="_blank" rel="noopener noreferrer" class="ease-snappy flex items-center p-3 rounded-xl border border-white/8 bg-white/5 text-white/80 transition-all duration-400 hover:border-white/15 hover:bg-white/10 hover:text-white group">
                <div class="p-2 rounded-lg bg-white/10 backdrop-blur-[20px] mr-3">
                  <social.icon size={16} class="text-accent-main" />
                </div>
                <div class="flex-1">
